...
export function MembershipReqTable({ me }: { me: number }) {
  ...
  const companyUsers = useQuery({
    ...
    queryFn: () =>
      axios.get("/api/users/list").then((res) => {
        const users = Object.values(res.data).filter(
          (user: any) => user.id !== me,
        );
  ...
  const changeUserStatus = (
    ...
    axios.patch("/api/users/info", { id: row.original.id, status }).then(() => {
      setUsers((users) =>
        users.map((u) => {
          if (u.id === row.original.id) {
            u.updatingTo = null;
  ...
  const cols: ColumnDef<UserChecked>[] = [
    {
      ...
      header: ({ table, column, header }) => {
        return (
          <Checkbox
            disabled={!companyUsers.isSuccess}
            checked={allChecked}
    ...
    },
    {
      ...
      enableSorting: true,
      cell: ({ row }) => (
        <span className="font-light">{row.original.email}</span>
      ),
    },
    {
      id: "status",
      ...
      cell: ({ row }) => {
        ...
        return (
          <div className="flex justify-center">
            <Button
              disabled={
                row.original.updatingTo !== null ||
                row.original.membershipStatus === "rejected"
              }
              onClick={() => {
                changeUserStatus(row, "rejected");
              }}
              size={"sm"}
            ...
            <Button
              disabled={
                row.original.updatingTo !== null ||
                row.original.membershipStatus === "accepted"
              }
              onClick={() => {
                changeUserStatus(row, "accepted");
              }}
