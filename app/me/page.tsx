...
export default function ProfileDetails() {
  const me = useQuery({
    queryKey: ["me"],
    queryFn: () => axios.get("/api/users/info")
      .then((res) => {
  ...
  if (me.isLoading) return <div className="flex justify-center items-center h-full">
    <Loader2 className="animate-spin" />
  </div>

  const spanPropDescriptor = (val: string) => <span className="text-sm font-light">{val}</span>
  const isCompAdmin = me.data && me.data.role === "companyAdmin"
  const isMembership = me.data && me.data.membershipStatus === "accepted"

  return <div className="flex flex-col py-8 gap-8 max-w-[100%]">
    <div className={`flex ${isCompAdmin ? "flex-row" : "flex-col"} flex-wrap gap-8 px-8 justify-center items-center`}>
      {me.isSuccess &&
        <>
          ...
          <SimpleCard
            ...
            headerActions={
              <div className="flex flex-col justify-around items-end gap-1">
                ...
                <Badge
                  className={cn("w-fit", st === "waiting" ? "bg-badge-neutral" : (st === "rejected" ? "bg-badge-error" : "bg-badge-ok"))}
                >
                  {enumMappings.memberShipStatuses[me.data.membershipStatus]}
                </Badge>
              </div>
            }
          >
